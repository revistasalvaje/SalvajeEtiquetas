        ...
        def generar_pdf():
            """Generate address labels PDF with optional offsets"""
            try:
                offset_x = float(request.args.get("offset_x", 0))
                offset_y = float(request.args.get("offset_y", 0))

                buffer = generate_address_labels(offset_x=offset_x, offset_y=offset_y)
        ...
        def generar_etiquetas_or():
            """Generate OR labels PDF with optional offsets"""
            try:
                offset_x = float(request.args.get("offset_x", 0))
                offset_y = float(request.args.get("offset_y", 0))

                buffer = generate_or_labels(offset_x=offset_x, offset_y=offset_y)
                return send_file(buffer,
                                 mimetype="application/pdf",
                                 as_attachment=False,
